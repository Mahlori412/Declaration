...
require_once "./connection.php";
    ...
    mysqli_query($conn,"UPDATE items set  stud_num='" . $_POST['stud_num'] . "', item_nam='" . $_POST['item_nam'] . "' ,description='" . $_POST['description'] . "' ,serial_num='" . $_POST['serial_num'] . "',lost='" . $_POST['lost'] . "' WHERE itemId='" . $_POST['itemId'] . "'");
     ...
     echo "<script>alert('You Have Updated successfull A Item!!! ');window.location='../viewItems.php'</script>";
     exit();
    }
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="lost" id="">
                                <option name="lost" value=""><?php echo $row["lost"]="Not Lost";?></option>
                                <option value="1">Lost</option>
                            </select>
                        </div>
                        <input type="hidden" name="itemId" value="<?php echo $row["itemId"]; ?>"/>
                        <input type="submit" class="btn btn-primary" value="Submit">
                        <a href="../viewItems.php" class="btn btn-default">Cancel</a>
                    </form>
                </div>
            </div>  
        </div>
